...
#     image = cv2.imread('data/train_images/002fc4e19.jpg',cv2.IMREAD_COLOR)
#     value = [
...
#         print(set(train).intersection(set(valid)))
#         assert(len(set(train).intersection(set(valid)))==0)

#         os.makedirs('data/split', exist_ok=True)
#         np.save('data/split/train%d_%d.npy'%(n,len(train)),train)
#         np.save('data/split/valid%d_%d.npy'%(n,len(valid)),valid)
...
#     os.makedirs('data/split', exist_ok=True)
#     np.save('data/split/test_train_%d.npy'%len(valid), valid)
#     np.save('data/split/test_valid_%d.npy'%len(train), train)
...
#         image = cv2.imread('data/train_images/%s'%(i), cv2.IMREAD_COLOR)
#         mask  = np.array([run_length_decode(r, height=256, width=1600, fill_value=1) for r in rle])
