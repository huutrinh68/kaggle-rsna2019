...
import src.cnn.dataset.mappings as mappings


def apply_window_policy(image, row, policy):
    if policy == 1:
        image1 = kaggle.apply_window(image, 40, 80) # brain
        image2 = kaggle.apply_window(image, 80, 200) # subdural
        image3 = kaggle.apply_window(image, row.WindowCenter, row.WindowWidth)
        image1 = (image1 - 0) / 80
        ...
        image1 = kaggle.apply_window(image, 40, 80) # brain
        image2 = kaggle.apply_window(image, 80, 200) # subdural
        image3 = kaggle.apply_window(image, 40, 380) # bone
        image1 = (image1 - 0) / 80
        image2 = (image2 - (-20)) / 200
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = kaggle.rescale_image(image, row.RescaleSlope, row.RescaleIntercept)
